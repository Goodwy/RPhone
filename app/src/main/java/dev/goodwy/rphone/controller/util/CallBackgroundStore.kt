...
object CallBackgroundStore {
    ...
    suspend fun saveDefault(context: Context, source: Uri): Boolean =
        withContext(Dispatchers.IO) {
            ...
            val manager = prefs(app)
            val imported = importImage(app, source) ?: return@withContext false
            manager.updateContactBackgroundEntriesSuspend(
    ...
    suspend fun clearBlocking(context: Context, contactId: String?, numbers: List<String>) {
        val app = context.applicationContext
        val manager = prefs(app)
        ...
        manager.updateContactBackgroundEntriesSuspend(updates)
        scheduleGarbageCollection(app, manager)
    }

    suspend fun carryBlocking(context: Context, fromContactId: String?, toContactId: String?, numbers: List<String>) {
        val app = context.applicationContext
        val manager = prefs(app)
        val keys = numberKeys(numbers)
    ...
    private suspend fun materialize(context: Context, manager: PreferenceManager, stored: String): String? {
        val effective = migrateIfNeeded(context, manager, stored) ?: return null
        return toModel(effective)
    }

    private suspend fun migrateIfNeeded(context: Context, manager: PreferenceManager, stored: String): String? {
        ...
        val imported = importImage(context, stored.toUri()) ?: return stored
    ...
    private suspend fun writePointers(
        manager: PreferenceManager,
        contactId: String?,
        keys: List<String>,
        value: String
    ...
    private suspend fun dropValue(manager: PreferenceManager, value: String) {
        val updates = mutableMapOf<String, String?>()
    ...
    private suspend fun replaceValue(manager: PreferenceManager, oldValue: String, newValue: String) {
        val updates = mutableMapOf<String, String?>()
        manager.getContactBackgroundEntries().forEach { (key, stored) ->
    ...
    private suspend fun pruneStaleIdKeys(context: Context, manager: PreferenceManager) {
