...
object RillStaticMotion {
    ...
    fun <T> screenEnter(): FiniteAnimationSpec<T> =
        tween(RillDurations.Long2, easing = RillEasing.Emphasized)

    fun <T> screenExit(): FiniteAnimationSpec<T> =
        tween(RillDurations.Medium4, easing = RillEasing.EmphasizedAccelerate)

    fun <T> screenEnterEffects(): FiniteAnimationSpec<T> =
        tween(RillDurations.Medium4, easing = RillEasing.EmphasizedDecelerate)

    fun <T> screenExitEffects(): FiniteAnimationSpec<T> =
        tween(RillDurations.Short4, easing = RillEasing.EmphasizedAccelerate)

    fun <T> ambientLoop(durationMillis: Int): FiniteAnimationSpec<T> =
        tween(durationMillis, easing = RillEasing.Linear)
}
...
object RillMotion {
    ...
    @Composable
    fun <T> screenTransition(): FiniteAnimationSpec<T> = RillStaticMotion.screenEnter()

    @Composable
    fun <T> screenTransitionEffects(): FiniteAnimationSpec<T> = RillStaticMotion.screenEnterEffects()
